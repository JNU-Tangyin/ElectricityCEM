...
def main():
    ...
    # Create a larger pool of seeds for monitoring to introduce realistic variance
    LARGER_MONITORING_POOL = np.random.RandomState(6202).randint(0, 2**32 - 1, size=500)
    ...
    history = {
        ...
        "company_baseline": [],
        "savings_vs_self": [],
        "max_reward_in_pop": [] # Added this line
    }
    ...
    for i in range(args.n_iter):
        # 采样种群
        seeds_pop = np.random.randint(0, 2**32-1, size=20)
        ...
        # 定期监控 (Sample from the larger pool for realistic variance)
        monitoring_seeds_this_iter = np.random.choice(LARGER_MONITORING_POOL, size=20, replace=False)
        d_r, s_r, c_r = eval_baselines(monitoring_seeds_this_iter)
        cem_r = evaluate(mu, monitoring_seeds_this_iter)
        ...
        history["max_reward_in_pop"].append(rewards.max()) # Log the best reward in the current population
